# Lambda: confirmForgotPassword (`POST /auth/forgot-password/confirm`)
...
- **Evento:** HTTP API (`POST /auth/forgot-password/confirm`)
...
1. A IA/Frontend envia `POST /auth/forgot-password/confirm` com `email`, `confirmationCode` e `password`.
2. O controller valida o payload com `confirmForgotPasswordSchema`.
3. O `ConfirmForgotPasswordUseCase` confirma a senha no Cognito via `AuthGateway`.
4. O frontend pode redirecionar o usuário para login após sucesso (204).
