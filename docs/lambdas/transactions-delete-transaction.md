# Lambda: deleteTransaction (`DELETE /entities/{entityId}/transactions/{transactionId}`)
...
- **Evento:** HTTP API (`DELETE /entities/{entityId}/transactions/{transactionId}`)
...
1. A IA/Frontend chama `DELETE /entities/{entityId}/transactions/{transactionId}` com `Authorization`.
2. O controller valida params e chama o use case.
3. O use case valida permissão e deleta a transação.
4. Resposta retorna status `204` sem body.
