  ...
  const quickFilters = [
    {
      id: "all" as const,
      label: "Em aberto",
      helper: `${payables?.openCount ?? 0} contas`,
    },
    {
      id: "overdue" as const,
      label: "Vencidas",
      helper: `${payables?.overdueCount ?? 0} contas`,
    },
    {
      id: "today" as const,
      label: "Hoje",
      helper: `${payables?.dueTodayCount ?? 0} contas`,
    },
    {
      id: "upcoming" as const,
      label: `Próximos ${horizonDays} dias`,
      helper: `${payables?.upcomingCount ?? 0} contas`,
    },
  ];
    ...
    <div className="flex flex-col gap-4 py-4 md:gap-6 md:py-6">
      <div className="px-4 lg:px-6">
        ...
        <p className="text-muted-foreground text-sm">
          Filtre vencimentos com um clique, visualize a concentracao por contato
          e marque contas como pagas diretamente nesta tela.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 px-4 lg:px-6 md:grid-cols-3">
        ...
        <Card>
          <CardHeader>
            ...
            <CardTitle>
              {formatMoney(
                (payables?.dueTodayTotal ?? 0) + (payables?.upcomingTotal ?? 0)
              )}
            </CardTitle>
          </CardHeader>
          <CardContent className="text-sm text-muted-foreground">
            {payables?.dueTodayCount ?? 0} para hoje e{" "}
            {payables?.upcomingCount ?? 0} nos proximos {horizonDays} dias.
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 px-4 lg:px-6 xl:grid-cols-3">
        <Card className="xl:col-span-1">
          <CardHeader>
            ...
            <CardDescription>
              Concentração das contas abertas no filtro atual.
            </CardDescription>
          </CardHeader>
        ...
        <Card className="xl:col-span-2">
          <CardHeader>
            ...
            <CardDescription>
              Use a ação rápida da tabela para marcar cada conta como paga sem
              sair desta visão.
            </CardDescription>
          </CardHeader>
